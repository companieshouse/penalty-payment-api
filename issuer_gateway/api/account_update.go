...
import (
	...
	utilities "github.com/companieshouse/penalty-payment-api/utils"
)
...
var getCompanyCodeFromTransaction = func(transactions []models.TransactionItem) (string, error) {
	return utilities.GetCompanyCodeFromTransaction(transactions)
}
...
func UpdateIssuerAccountWithPenaltyPaid(client *e5.Client, resource models.PayableResource, payment validators.PaymentInformation) error {
	...
	if err != nil {
		if svcErr := RecordIssuerCommandError(resource, e5.CreateAction); svcErr != nil {
	...
	if err != nil {
		if svcErr := RecordIssuerCommandError(resource, e5.AuthoriseAction); svcErr != nil {
			log.Error(svcErr, log.Data{"payment_id": payment.PaymentID, "penalty_reference": resource.Reference})
			return err
		}
	...
	if err != nil {
		if svcErr := RecordIssuerCommandError(resource, e5.ConfirmAction); svcErr != nil {
			log.Error(svcErr, log.Data{"payment_id": payment.PaymentID, "penalty_reference": resource.Reference})
			return err
		}
...
func RecordIssuerCommandError(resource models.PayableResource, action e5.Action) error {
