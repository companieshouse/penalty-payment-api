...
func TransactionsArePayable(companyNumber string, companyCode string, txs []models.TransactionItem) ([]models.TransactionItem, error) {
	response, _, err := service.GetPenalties(companyNumber, companyCode)
	if err != nil {
		log.Error(err)
		return nil, err
	}
