...
type PaymentDetailsService struct {
	PayableResourceService *common.PayableResourceService // needed when implementing PATCH endpoint from payment-processed-consumer
}
...
func (service *PaymentDetailsService) GetPaymentDetailsFromPayableResource(req *http.Request,
	payable *models.PayableResource, penaltyDetailsMap *config.PenaltyDetailsMap, companyCode string) (*models.PaymentDetails, common.ResponseType, error) {
	...
	if len(paymentDetails.Items) == 0 {
		...
		return nil, common.InvalidData, err
	}
	return paymentDetails, common.Success, nil
}
