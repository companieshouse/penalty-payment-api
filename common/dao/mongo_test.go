...
//			}
//
//			mockCollection.EXPECT().FindOne(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(&result, nil)
//
//			err := svc.CreateAccountPenalties(dao)
//
//			So(err, ShouldBeNil)
...
//
//			err := svc.GetAccountPenalties(dao)
//
//			So(err, ShouldBeNil)
//		})
//
...
//			mockCollection.EXPECT().FindOne(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil)
//
//			err, _ := svc.GetPayableResource(customerCode, payableRef)
//
//			So(err, ShouldBeNil)
//		})
//
