...
type PayableResourceDaoService interface {
	...
	CreatePayableResource(dao *models.PayableResourceDao, ctx string) error
	// GetPayableResource will find a single payable resource with the given customerCode and payableRef
	GetPayableResource(customerCode, payableRef string, ctx string) (*models.PayableResourceDao, error)
	// UpdatePaymentDetails will update the resource with changed values
	UpdatePaymentDetails(dao *models.PayableResourceDao, ctx string) error
	// SaveE5Error stored which command to E5 failed e.g. create, authorise or confirm
	SaveE5Error(customerCode, payableRef string, ctx string, action e5.Action) error
...
type AccountPenaltiesDaoService interface {
	...
	CreateAccountPenalties(dao *models.AccountPenaltiesDao, ctx string) error
	// GetAccountPenalties will find the account penalties for a given customerCode and companyCode
	GetAccountPenalties(customerCode string, companyCode string, ctx string) (*models.AccountPenaltiesDao, error)
	// UpdateAccountPenaltyAsPaid will update a transactions as paid for a given customerCode, companyCode and penaltyRef
	UpdateAccountPenaltyAsPaid(customerCode string, companyCode string, penaltyRef string, ctx string) error
	// UpdateAccountPenalties will update the created_at, closed_at and data fields of an existing document
	UpdateAccountPenalties(dao *models.AccountPenaltiesDao, ctx string) error
}
